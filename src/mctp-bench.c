...
struct recv_ctx {
        ...
        unsigned char *buf;
        volatile bool started_recv_flag;
        int sd;
};
...
{
        float throughput =
            get_throughput((float)recv_ctx->stats.total_received_len,
                           (float)recv_ctx->stats.elapsed_time);
...
static int get_elapsed_time(struct recv_ctx *recv_ctx)
{
...
static int handle_incoming_msg(struct recv_ctx *recv_ctx)
{
        ...
        if (recv_ctx->stats.curr_packet_len > MAX_LEN) {
                warn("recv: expected max len:%zd bytes, got:%zd bytes",
                     MAX_LEN, recv_ctx->stats.curr_packet_len);
                return -1;
        }
