...
#define ERROR_BRIDGE_START_EID 0
...
//each bit representing endpoints in the network EID pool
typedef struct {
...
// Add range of EIDs for a set
void eid_set_add_range(reserved_eid_set *set, uint8_t start_eid,
		       uint8_t end_eid)
{
	if (start_eid >= eid_alloc_min && end_eid <= eid_alloc_max) {
		for (uint8_t e = start_eid; e <= end_eid; e++) {
			eid_set_add(set, e);
		}
	}
}

// Remove range of EIDs for a set
void eid_set_remove_range(reserved_eid_set *set, uint8_t start_eid,
			  uint8_t end_eid)
{
	if (start_eid >= eid_alloc_min && end_eid <= eid_alloc_max) {
		for (uint8_t e = start_eid; e <= end_eid; e++) {
			eid_set_remove(set, e);
		}
	}
}
...
static int remove_peer(struct peer *peer)
{
	...
	if (peer->pool_size > 0) {
		...
		for (mctp_eid_t e = peer->pool_start;
		     e < peer->pool_start + peer->pool_size; e++) {
			if (eid_set_contains(n->resv_eid, e)) {
				eid_set_remove(n->resv_eid, e);
				warnx("remove from reserve for net %d eid %d",
				      peer->net, e);
			}
		}
	}
...
static int endpoint_assign_eid(struct ctx *ctx, sd_bus_error *berr,
			       ...
			       mctp_eid_t static_eid, bool check_bridge)
{
	...
	if (static_eid) {
		...
		for (e = eid_alloc_min; e <= eid_alloc_max; e++) {
			if (n->peers[e])
				continue;
			// todo: check reservation of eid e from other bridge
			if (check_bridge) {
				// check for max sized pool as well from e + 1
				int max_pool_size = ctx->max_pool_size;
				int num_downstream_eid = 0;
				if (e + 1 + max_pool_size > eid_alloc_max) {
					warnx("Ran out of EIDs for net %d, allocating %s",
					      net, dest_phys_tostr(dest));
					sd_bus_error_setf(berr,
							  SD_BUS_ERROR_FAILED,
							  "Ran out of EIDs");
					return -EADDRNOTAVAIL;
				}
				for (; num_downstream_eid < max_pool_size &&
				       (e + num_downstream_eid + 1) <=
					       eid_alloc_max;) {
					//check reservation of EID from another bridge
					if (n->peers[e + num_downstream_eid +
						     1] != NULL ||
					    eid_set_contains(
						    n->resv_eid,
						    (e + num_downstream_eid +
						     1))) {
						break;
					}
					num_downstream_eid++;
				}
				if (num_downstream_eid == max_pool_size) {
					rc = add_peer(ctx, dest, e, net, &peer);
					if (rc < 0)
						return rc;
					peer->pool_size = max_pool_size;
					peer->pool_start = e + 1;
					break;
				} else {
					e = e + num_downstream_eid;
					continue;
				}
			}
			rc = add_peer(ctx, dest, e, net, &peer);
		...
		if (e > eid_alloc_max ||
		    (check_bridge && (e == eid_alloc_max))) {
			if (check_bridge) {
				warnx("Ran out of EIDs for net %d, allocating Bridge at %s ",
				      net, dest_phys_tostr(dest));
			} else {
				warnx("Ran out of EIDs for net %d, allocating %s",
				      net, dest_phys_tostr(dest));
	...
	// reserve eids from peer->pool_start -> peer->pool_start + max_pool_size
	if (check_bridge && (peer->pool_size == ctx->max_pool_size)) {
		eid_set_add_range(n->resv_eid, peer->pool_start,
	...
	// release extra reserved eids (max_pool_size - peer->pool_size)
	if (check_bridge && (peer->pool_size <= ctx->max_pool_size)) {
		eid_set_remove_range(n->resv_eid,
				     peer->pool_start + peer->pool_size,
				     peer->pool_start + ctx->max_pool_size - 1);
	}
	if (new_eid != peer->eid) {
		// release all earlier reserved eid from
		// peer->pool_start -> peer->pool_start + max_pool_size
		if (check_bridge) {
			eid_set_remove_range(n->resv_eid, peer->pool_start,
					     peer->pool_start +
						     peer->pool_size - 1);
		}
		...
		// check again for available peer->pool_size from new peer->eid + 1
		if (check_bridge && (peer->pool_size > 0)) {
			if (peer->eid + peer->pool_size > eid_alloc_max) {
				warnx("Error: Bridge assigned eid %d but ran out of EIDs for its downstream endpoints",
				      peer->eid);
				sd_bus_error_setf(berr, SD_BUS_ERROR_FAILED,
						  "Ran out of EIDs");
				peer->pool_start =
					ERROR_BRIDGE_START_EID; // using reserved eid for error
			} else {
				for (e = peer->eid + 1;
				     e <= (peer->eid + peer->pool_size); e++) {
					//add: check reservation of eid e from other bridge
					if (n->peers[e] != NULL ||
					    eid_set_contains(n->resv_eid, e)) {
						warnx("Error: Bridge assigned eid %d but failed to find contigous pool",
						      peer->eid);
						peer->pool_start =
							ERROR_BRIDGE_START_EID; // using reserved eid for error
						break;
				...
				if (peer->pool_start !=
				    ERROR_BRIDGE_START_EID) {
					// reserved eids from new peer->eid + 1 -> peer->eid + peer->pool_size
					eid_set_add_range(
						n->resv_eid, peer->eid + 1,
						peer->eid + peer->pool_size);
					 ...
					 sd_bus_error *berr)
{
	...
	if (peer) {
		...
		if (n->resv_eid && eid_set_contains(n->resv_eid, eid)) {
			return sd_bus_error_setf(
				berr, SD_BUS_ERROR_INVALID_ARGS,
				"EID %d is marked as reserved via another MCTP Bridge",
				eid);
		}
	}
...
static int change_net_interface(struct ctx *ctx, int ifindex, uint32_t old_net)
{
	...
	for (size_t i = 0; i < ctx->num_peers; i++) {
		...
		if (old_n->resv_eid) {
			...
			warnx("remove from reserve for net %d eid %d", old_net,
			      peer->eid);
		}
		if (new_n->resv_eid) {
...
static int add_net(struct ctx *ctx, uint32_t net_id)
{
	...
	if (!net->resv_eid) {
		warnx("Failed to allocate reserved_eid_set for net %d",
		      net->net);
		return -ENOMEM;
	}
	...
	if (rc) {
		...
		for (mctp_eid_t e = peer->pool_start;
			...
			if (n->resv_eid) {
				...
				warnx("adding to reserve for net %d eid %d",
				      peer->net, e);
			}
		}
