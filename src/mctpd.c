...
bool find_local_eids_count_by_net(ctx *ctx, uint32_t net, size_t *num)
{
	net_det *n = lookup_net(ctx, net);
	peer *peer;
	*num = 0;
...
mctp_eid_t * find_local_eids_by_net(ctx *ctx, uint32_t net,
                                    const size_t local_eid_cnt)
{
	mctp_eid_t *ret_eids = (mctp_eid_t *)malloc(local_eid_cnt);
		...
		sd_bus_message *reply, void *userdata, sd_bus_error *berr)
{
	...
	if (strcmp(property, "LocalEids") == 0) {
		if (!find_local_eids_count_by_net(ctx, netid, &num))
			return -ENOENT;

		eids = find_local_eids_by_net(ctx, netid, num);
		dfree(eids);
